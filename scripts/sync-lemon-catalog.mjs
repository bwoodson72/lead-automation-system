...
if (!apiKey || !storeId) {
  throw new Error("LEMONSQUEEZY_API_KEY and LEMONSQUEEZY_STORE_ID are required to sync the catalog.");
}
...
async function fetchCatalogPage(url) {
  ...
  if (!response.ok) {
    throw new Error(`Lemon Squeezy catalog request failed with HTTP ${response.status}.`);
  }
...
async function clearGeneratedImages() {
  ...
  await Promise.all(
    entries
      .filter((entry) => entry.isFile() && /^\d+\.(?:avif|jpe?g|png|webp)$/.test(entry.name))
      .map((entry) => rm(path.join(GENERATED_IMAGE_DIRECTORY, entry.name))),
  );
}
...
async function downloadProductImage(productId, imageUrls) {
  ...
  for (const imageUrl of candidates) {
    try {
      ...
      if (!extension) {
        lastFailure = "unsupported image content type";
        continue;
      }

      const declaredSize = Number(response.headers.get("content-length") ?? 0);
      if (declaredSize > MAX_IMAGE_BYTES) {
        lastFailure = "image exceeds the 12 MB limit";
        continue;
      }

      const bytes = Buffer.from(await response.arrayBuffer());
      if (bytes.length === 0 || bytes.length > MAX_IMAGE_BYTES) {
        lastFailure = "invalid image size";
        continue;
      }
...
const products = await Promise.all(
  lemonProducts.map(async ({ id, attributes }) => {
